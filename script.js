...
document.addEventListener('DOMContentLoaded', () => {
    // Si ya hay un usuario logueado en sesión activa, lo cargamos
    const storedName = localStorage.getItem('fp_name');
    if (storedName) {
        // Opcional: Podrías hacer que requiera login cada vez que refresca la página
        // quitando estas líneas. Pero es mejor UX mantenerlo.
        userName = storedName;
        updateUserDisplay();
    }
...
function login() { 
    ...
    // 1. Obtener datos reales guardados
    ...
    // CASO A: No existe cuenta registrada en este navegador
    if (!storedEmail) {
        showStatusMsg("No existe cuenta registrada. Por favor crea una.", "error");
        return;
    }

    // CASO B: El correo no coincide
    if (inputEmail !== storedEmail) {
        showStatusMsg("Correo no registrado.", "error");
        return;
    }

    // CASO C: La contraseña no coincide (Cualquier contraseña errónea da error)
    if (inputPass !== storedPass) {
        showStatusMsg("Contraseña incorrecta.", "error");
        return;
    }

    // CASO D: Todo coincide -> Éxito
...
// -----------------------------------------------------
// --- LOGICA DE REGISTRO (GUARDA LOS DATOS) ---
// -----------------------------------------------------
function register() { 
    ...
    emailjs.send(EMAIL_SERVICE_ID, EMAIL_TEMPLATE_WELCOME_ID, templateParams)
        ...
        .finally(() => {
            // AQUÍ GUARDAMOS LOS DATOS OBLIGATORIOS PARA EL LOGIN
            localStorage.setItem('fp_email', email);
            localStorage.setItem('fp_pass', pass); 
...
function recoverPassword() {
    ...
    // Verificamos si el correo existe antes de enviar código
    const storedEmail = localStorage.getItem('fp_email');
    ...
    // Opcional: Solo enviar si el correo está registrado
    if (storedEmail && email !== storedEmail) {
        showStatusMsg("Este correo no está registrado en el sistema.", "error");
        return;
    }
    ...
    emailjs.send(EMAIL_SERVICE_ID, EMAIL_TEMPLATE_RECOVERY_ID, templateParams)
        .then(function(response) {
            ...
            showStatusMsg("Error al enviar.", "error");
        })
        .finally(() => {
            btn.innerText = originalText;
            btn.disabled = false;
