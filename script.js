...
// Recuerda crear tu plantilla de bienvenida y pegar su ID aquí abajo:
...
const menuItems = [
    ...
    { id: 3, category: 'principal', name: "Porotos con Riendas", price: 4800, provider: "FreshPoroto", img: "https://images.unsplash.com/photo-1594968461783-a7457b01b0f4?auto=format&fit=crop&w=400" },
    { id: 4, category: 'principal', name: "Charquicán", price: 4900, provider: "FreshPoroto", img: "https://images.unsplash.com/photo-1627308595229-7830a5c91f9f?auto=format&fit=crop&w=400" },
    ...
    { id: 6, category: 'principal', name: "Lomo a lo Pobre", price: 8500, provider: "FreshPoroto", img: "https://images.unsplash.com/photo-1544378730-8b5104b13790?auto=format&fit=crop&w=400" },
    { id: 7, category: 'principal', name: "Humitas (2u)", price: 5000, provider: "FreshPoroto", img: "https://images.unsplash.com/photo-1626315220367-933df9449072?auto=format&fit=crop&w=400" },
    ...
    { id: 9, category: 'principal', name: "Paila Marina", price: 7500, provider: "FreshPoroto", img: "https://images.unsplash.com/photo-1579887829663-67706e62e6ac?auto=format&fit=crop&w=400" },
    { id: 11, category: 'principal', name: "Lasaña Boloñesa", price: 5800, provider: "La Nonna", img: "https://images.unsplash.com/photo-1574868352513-035293653bf0?auto=format&fit=crop&w=400" },
    ...
    { id: 18, category: 'principal', name: "Lomo Saltado", price: 7200, provider: "Sabor Peruano", img: "https://images.unsplash.com/photo-1582234372723-5e79d8544d65?auto=format&fit=crop&w=400" },
...
let generatedRecoveryCode = null; // Variable para guardar el código

document.addEventListener('DOMContentLoaded', () => {
    // Si ya hay un usuario guardado, cargarlo (Opcional, pero recomendado)
    const storedName = localStorage.getItem('fp_name');
    if (storedName) {
        userName = storedName;
        updateUserDisplay();
    }
...
function openAuthModal(mode = 'login') {
    ...
    // Limpiar input de código si quedó sucio
...
function switchAuth(mode) {
    ...
    // Ocultar pestañas si es recuperación O validación de código
...
// -----------------------------------------------------
// --- LOGICA DE LOGIN ACTUALIZADA (CON VALIDACIÓN) ---
// -----------------------------------------------------
function login() { 
    ...
    if(inputEmail === "" || inputPass === "") {
        showStatusMsg("Por favor completa todos los campos.", "error");
        return;
    }

    // 1. Obtener datos guardados en LocalStorage (simulando Base de Datos)
    const storedEmail = localStorage.getItem('fp_email');
    const storedPass = localStorage.getItem('fp_pass');
    const storedName = localStorage.getItem('fp_name');

    // 2. Validar credenciales
    if (storedEmail && inputEmail === storedEmail && inputPass === storedPass) {
         showStatusMsg(`¡Bienvenido de nuevo, ${storedName}!`, "success");
         setTimeout(() => {
            userName = storedName; 
            closeAuthModal();
            updateUserDisplay();
        ...
        // Si no coincide o no existe usuario
        showStatusMsg("Correo o contraseña incorrectos.", "error");
    }
}

// -----------------------------------------------------
// --- LOGICA DE REGISTRO ACTUALIZADA (GUARDAR DATOS) ---
// -----------------------------------------------------
function register() { 
    ...
    if(name === "" || email === "" || pass === "") {
        showStatusMsg("Todos los campos son obligatorios.", "error");
        return;
    }
    if(!email.includes('@')) {
        showStatusMsg("Ingresa un correo válido.", "error");
        return;
    }
    ...
    // Enviar correo bienvenida (si falla, igual registra)
    const templateParams = { destinatario: email, name: name };

    emailjs.send(EMAIL_SERVICE_ID, EMAIL_TEMPLATE_WELCOME_ID, templateParams)
        .then(() => console.log('Welcome email sent'))
        .catch(err => console.log('Email failed', err))
        .finally(() => {
            // 1. GUARDAR DATOS DEL USUARIO EN EL NAVEGADOR
            localStorage.setItem('fp_email', email);
            localStorage.setItem('fp_pass', pass); // Nota: En una app real, ¡nunca guardes contraseñas así!
...
function recoverPassword() {
    ...
    if(!email || !email.includes('@')) {
        showStatusMsg("Por favor, ingresa un correo válido.", "error");
        return;
    }
    ...
    // 1. Generar código y guardarlo en variable global
    generatedRecoveryCode = Math.floor(100000 + Math.random() * 900000); 

    const templateParams = {
        destinatario: email,
        recovery_code: generatedRecoveryCode,
        reply_to: email          
    };

    emailjs.send(EMAIL_SERVICE_ID, EMAIL_TEMPLATE_RECOVERY_ID, templateParams)
        .then(function(response) {
            console.log('SUCCESS!', response.status, response.text);
            showStatusMsg(`Código enviado a ${email}`, "success");
            
            // 2. EN LUGAR DE CERRAR, CAMBIAMOS A PANTALLA DE CÓDIGO
            setTimeout(() => {
                switchAuth('code-validation');
                showStatusMsg("Revisa tu correo e ingresa el código.", "success");
            }, 1500);

        }, function(error) {
            console.log('FAILED...', error);
            showStatusMsg("Error al enviar. Verifica tu conexión.", "error");
        })
        .finally(() => {
            btn.innerText = originalText;
            btn.disabled = false;
...
// --- NUEVA FUNCIÓN: VALIDAR CÓDIGO ---
function validateCode() {
    const input = document.getElementById('input-code').value;
    
    if(!input) {
        showStatusMsg("Por favor escribe el código.", "error");
        return;
    }

    // Comparamos el input con la variable guardada
    ...
    if(input == generatedRecoveryCode) {
        ...
        // Simular cambio de clave (actualizar localStorage si quisieras)
        // Por ahora, solo logueamos al usuario
        setTimeout(() => {
            switchAuth('login');
            showStatusMsg("Ingresa con tu contraseña.", "success");
            generatedRecoveryCode = null; // Limpiar código por seguridad
...
function logout() { 
    // Opcional: localStorage.clear(); // Si quieres borrar los datos al salir (no recomendado si quieres persistencia)
    location.reload(); 
}
...
function processOrder() {
    ...
    setTimeout(() => {
        let msg = deliveryType === 'delivery' ? '¡Tu Delivery está confirmado! Se enviará a tu dirección.' : '¡Pedido listo para retiro! Pasa por caja.';
        alert(msg);
