...
// TUS PLANTILLAS
const EMAIL_TEMPLATE_RECOVERY_ID = 'template_ouhyyvm'; // ID para recuperar clave
// ¡OJO! Aquí debes poner la ID de la nueva plantilla que crees para la bienvenida:
const EMAIL_TEMPLATE_WELCOME_ID = 'template_bu3kgqf'; 
...
// ======================================================
// --- 2. DATOS Y LÓGICA DEL RESTAURANTE ---
// ======================================================
...
document.addEventListener('DOMContentLoaded', () => {
    renderMenu();
    showSection('about'); // Empezar en Historia
});

// --- AUTH ---
function openAuthModal(mode = 'login') {
    document.getElementById('auth-modal').style.display = 'flex';
    document.getElementById('auth-msg').style.display = 'none'; // Limpiar mensajes
    document.getElementById('form-recovery').classList.remove('active');
    // Reset recovery view if open
    switchAuth(mode);
}
...
function switchAuth(mode) {
    ...
    // Manage Tabs visibility (hide tabs if recovery)
...
function login() { 
    ...
    showStatusMsg("¡Bienvenido! Iniciando sesión...", "success");
    setTimeout(() => {
        userName = "Benjamin"; 
        closeAuthModal();
        updateUserDisplay();
...
// -----------------------------------------------------
// --- FUNCIÓN REGISTRO (CON BIENVENIDA) ---
// -----------------------------------------------------
function register() { 
    ...
    // 3. Preparar correo de bienvenida
    const templateParams = {
        destinatario: email,  // A quién se le envía
        name: name            // El nombre para el saludo "Hola Juan"
    };

    // 4. Enviar correo y finalizar registro
    // Nota: Si no has puesto el ID de la plantilla de bienvenida arriba, esto dará error.
    emailjs.send(EMAIL_SERVICE_ID, EMAIL_TEMPLATE_WELCOME_ID, templateParams)
        .then(function(response) {
            ...
            userName = name;
            showStatusMsg("¡Cuenta creada! Te hemos enviado un correo de bienvenida.", "success");
            
            // Entrar a la app después de 2 segundos
            setTimeout(() => {
                closeAuthModal();
                updateUserDisplay();
                // Limpiar formulario
            ...
            // Si falla el correo, igual dejamos entrar al usuario (es mejor para la UX)
            userName = name;
            showStatusMsg("Cuenta creada (pero no pudimos enviarte el correo).", "success");
            setTimeout(() => {
                closeAuthModal();
                updateUserDisplay();
            }, 2000);
...
// -----------------------------------------------------
// --- FUNCIÓN DE RECUPERACIÓN (YA FUNCIONAL) ---
// -----------------------------------------------------
function recoverPassword() {
    ...
    // 3. Generar un código o link simulado
    const fakeCode = Math.floor(100000 + Math.random() * 900000); // Código de 6 dígitos

    // 4. Parámetros para la plantilla de EmailJS
    const templateParams = {
        destinatario: email,     // Variable en tu template
        recovery_code: fakeCode, // Variable en tu template
        reply_to: email          
    };
    ...
    emailjs.send(EMAIL_SERVICE_ID, EMAIL_TEMPLATE_RECOVERY_ID, templateParams)
        .then(function(response) {
            ...
            showStatusMsg(`¡Listo! Hemos enviado un código a ${email}`, "success");
            emailInput.value = ""; // Limpiar campo
            
            // Regresar al login automáticamente
            setTimeout(() => {
                switchAuth('login');
                showStatusMsg("Usa tu código temporal para ingresar (Simulado)", "success");
            }, 4000);
...
function updateUserDisplay() {
    // Mostrar panel de usuario, ocultar botones de auth
    ...
    // Ocultar botón de login móvil si existe
...
function processOrder() {
    ...
    setTimeout(() => {
        let msg = deliveryType === 'delivery' 
            ? '¡Tu Delivery está confirmado! Se enviará a tu dirección.' 
            : '¡Pedido listo para retiro! Pasa por caja.';
