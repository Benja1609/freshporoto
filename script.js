...
// ======================================================
// Aquí guardamos los usuarios como si fuera una base de datos real
...
function saveUserToDB(user) {
    ...
    if (!exists) {
        ...
        // Si existe, podríamos actualizarlo, pero por seguridad en registro no hacemos nada
        // o podríamos manejar error, pero register() ya valida antes.
    }
}

document.addEventListener('DOMContentLoaded', () => {
    // Verificar si hay una SESIÓN ACTIVA (no solo datos guardados)
    const activeSessionEmail = localStorage.getItem('fp_active_session_email');
    
    if (activeSessionEmail) {
        // Buscar los datos de ese usuario en la BD
        const users = getUsersDB();
        ...
        if (user) {
            ...
            // Si hay sesión pero no encontramos al usuario (raro), cerramos
            logout(); 
        }
    }
...
function login() { 
    ...
    // 3. Validar
    if (!user) {
        showStatusMsg("Este correo no está registrado.", "error");
        return;
    }
    ...
    // 4. Éxito: Crear Sesión Activa
    localStorage.setItem('fp_active_session_email', user.email); // Marca quién está logueado
...
function register() { 
    ...
    // Verificar si ya existe antes de enviar correo
    const users = getUsersDB();
    if(users.find(u => u.email === email)) {
        showStatusMsg("Este correo ya está registrado. Intenta iniciar sesión.", "error");
        return;
    }
...
// -----------------------------------------------------
// --- RECUPERACIÓN (VALIDA SI EXISTE CORREO) ---
// -----------------------------------------------------
...
// -----------------------------------------------------
// --- UI Y NAVEGACIÓN (LOGOUT CORREGIDO) ---
// -----------------------------------------------------
...
// --- LOGOUT CORREGIDO ---
function logout() { 
    // Solo borramos la "sesión activa", NO la base de datos de usuarios
