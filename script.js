...
const EMAIL_TEMPLATE_WELCOME_ID = 'PEGAR_AQUI_TU_NUEVA_TEMPLATE_ID_SI_LA_TIENES'; 
const EMAIL_TEMPLATE_ORDER_ID = 'template_order_confirm'; // <--- CREAR ESTE ID EN EMAILJS

(function() {
    // IMPORTANTE: Descomenta la siguiente línea para que el envío de correos funcione de verdad
...
const menuItems = [
    ...
    // ID 2: IMAGEN CORREGIDA (SOPA/CAZUELA)
    { id: 2, category: 'principal', name: "Cazuela de Vacuno", price: 5200, provider: "FreshPoroto", img: "https://images.unsplash.com/photo-1547592180-85f173990554?auto=format&fit=crop&w=400" },
    ...
    // ID 5: IMAGEN CORREGIDA (CARNE MECHADA/AL JUGO)
    { id: 5, category: 'principal', name: "Carne Mechada", price: 6500, provider: "FreshPoroto", img: "https://images.unsplash.com/photo-1600891964092-4316c288032e?auto=format&fit=crop&w=400" },
...
function register() { 
    ...
    // Simulación de éxito en registro
    Promise.resolve() 
        .then(() => {
...
// --- FUNCIÓN PRINCIPAL DE PEDIDO ACTUALIZADA ---
function processOrder() {
    // 1. Obtener datos del usuario actual
    const currentUserEmail = localStorage.getItem('fp_active_session_email') || "invitado@freshporoto.cl";
    
    // 2. Validaciones de Delivery (DIRECCIÓN DETALLADA)
    let fullAddress = "Retiro en Local";
    
    if (deliveryType === 'delivery') {
        const street = document.getElementById('addr-street').value;
        const num = document.getElementById('addr-num').value;
        const comuna = document.getElementById('addr-comuna').value;
        const phone = document.getElementById('user-phone').value;

        if (!street || !num || !comuna) { alert("Por favor completa Calle, Número y Comuna."); return; }
        if (!phone || phone.length < 8) { alert("Por favor ingresa un teléfono válido."); return; }
        
        fullAddress = `${street} #${num}, ${comuna}`;
    }
    ...
    if (paymentType === 'transfer') {
        ...
        if (!confirmed) {
            alert("Por favor, confirma que realizaste la transferencia.");
            return;
        }
    }
    ...
    let orderSummary = myOrders.map(item => `- ${item.name} ($${item.price})`).join('\n');
    let totalAmount = document.getElementById('total-price').innerText;

    const emailParams = {
        destinatario: currentUserEmail,    
        nombre_cliente: userName,          
        pedido_detalle: orderSummary,      
        total_pago: totalAmount,           
        tipo_entrega: deliveryType === 'delivery' ? 'Despacho a Domicilio' : 'Retiro en Tienda',
        direccion: fullAddress,            
        metodo_pago: paymentType === 'transfer' ? 'Transferencia' : 'Efectivo/Contra entrega'
    };

    // --- ENVIAR CORREO CON EMAILJS ---
    emailjs.send(EMAIL_SERVICE_ID, EMAIL_TEMPLATE_ORDER_ID, emailParams)
        .then(() => {
            alert(`✅ ¡CONFIRMADO!\n\nHemos enviado el comprobante a: ${currentUserEmail}\n\nTu pedido está siendo preparado.`);
            finalizeOrderCleanup();
        })
        .catch((error) => {
            console.error('Error enviando correo:', error);
            // Igual confirmamos el pedido aunque falle el correo
            alert("✅ Pedido recibido (No se pudo enviar el correo de confirmación, pero ya estamos cocinando).");
            finalizeOrderCleanup();
        })
        .finally(() => {
...
function finalizeOrderCleanup() {
    ...
    // Limpiar inputs
    if(document.getElementById('addr-street')) document.getElementById('addr-street').value = '';
    if(document.getElementById('addr-num')) document.getElementById('addr-num').value = '';
    if(document.getElementById('addr-comuna')) document.getElementById('addr-comuna').value = '';
    document.getElementById('user-phone').value = '';
    document.getElementById('transfer-confirmed').checked = false;
    
    // Resetear toggles
    setDeliveryMode('pickup', document.querySelector('.toggle-delivery')); 
    setPaymentMode('cash', document.querySelector('.toggle-payment'));
}